...
const vm = new Vue({
    ...
    mounted: async function () {
        ...
        //     'product_theme_id': this.album._id,
        //     'category': '圆形'
        // };
        // const path = window.theme.path;
        // this.uploadData.subpath = path.startsWith('/assets/images') ? window.theme.path.slice(14) : path;
    ...
    watch: {
        category: async function (category) {
            ...
            this.uploadData.category = category;
 
            const album = this.album;
            const url = `/api/album-images?album_id=${album._id}&album_code=${album.code}&category=${category}`;
	...
	methods: {
        ...
        handleScanFolder: async function () {
            const params = {
                path: this.theme.path,
                product_theme_id: this.theme.id
            };
            const config = { params: params};
            const response = await axios.get('/api/product/theme/image/scan', config);
            this.images = response.data;
        },
        handleRefresh: async function () {
            window.location.reload();
        },
        handleShowDirSelector: async function () {
            // @TODO 方案：使用nodejs读取本地目录数据，返回前端来展示，相当于是一个简易的文件管理器
        }
	}
});
