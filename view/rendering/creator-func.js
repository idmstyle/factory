...
function createImages_H(tplCode, originalTplImage, originalSources, isMixed = false) {
    ...
    for(let index = 0; index < sources.length; ) {
        ...
        let sourceCode = '';
        let map_areas = typeof tplImage.map_areas == 'string' ? JSON.parse(tplImage.map_areas) : tplImage.map_areas;
        for(let map_area of map_areas) {
            // 避免直接对 tplImage.map_areas 进行修改
            let area = JSON.parse(JSON.stringify(map_area));

            sourceCode = sources[index]['base'];
            area['id'] = 'image-' + sourceCode;
        ...
        console.log('image.sources:', image.sources);
