class TemplateShapeService {
    db;

    constructor() {
        this.dbName = '_dmstyle_factory_template_shapes';
    ...
    async findAll(options) {
        ...
        // 如果没有检索条件，则获取所有数据
        const selector = options.selector || { shape_text: {$exists: true} };

        // 默认使用 shape_text 进行排序， pouchdb 的语法要求排序的字段必须包含在 selector 中
        if( !selector.shape_text ) selector.shape_text = {$exists: true};
        ...
        const index = { fields: Object.keys(selector) };
